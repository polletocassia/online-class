# Diário de Sprint

### Divisão das responsabilidades
<ul>
    <li>
        Cássia
        <ol type="1">
            <li>Definição do problema;</li>
            <li>Definição das regras de negócio;</li>
            <li>Elaboração das User Stories;</li>
            <li>Criação do repositório GitHub;</li>
            <li>Criação do backlog;</li>
            <li>Modelo C4;</li>
            <li>Decisões arquiteturais.</li>
        </ol>
    </li>
    <li>
        Luisa
        <ol type="1">
            <li>Identificação dos stakeholders;</li>
            <li>Definição do MVP;</li>
            <li>BPMN;</li>
            <li>Organização da apresentação.</li>
        </ol>
    </li>
    <li>
        Pedro
        <ol type="1">
            <li>Diagrama de Casos de Uso;</li>
            <li>Arquitetura de Software;</li>
            <li>Organização do quadro Kanban;</li>
            <li>Diário de Sprint;</li>
            <li>Gestão do repositório GitHub.</li>
        </ol>
    </li>
</ul>

### Principais decisões tomadas
<ul>
    <li>Separação de responsabilidades e organização contínua.</li>
    <li>Busca por auxílio e tira dúvidas com a professora.</li>
    <li>Constantes pesquisas para aprofundar a compreensão de cada conteúdo abordado no projeto.</li>
    <li>Busca por melhorias no entrosamento da equipe.</li>
</ul>

### Dificuldades encontradas
<ul>
    <li>Engrenar o desenvolvimento do projeto de maneira fluida.</li>
    <li>Manutenção e organização das tarefas.</li>
    <li>Montagem do modelo C4.</li>
    <li>Diálogo entre os membros da equipe como reunião.</li>
    <li>Estruturar determinadas partes do modelo BPMN.</li>
</ul>

### Soluções adotadas
<ul>
    <li>Não desistir nas primeiras adversidades.</li>
    <li>Manter firmeza no aprendizado contínuo.</li>
    <li>Solicitar apoio quando necessário.</li>
    <li>Utilização da ferramenta para apoio no acompanhamento de tais tarefas.</li>
    <li>Estudar sobre modelos C4.</li>
    <li>Criação de grupo no Google Chat para compartilhamento de arquivos no Google Drive.</li>
    <li>Criação de grupo no WhatsApp para comunicação ágil, fácil e coloquial.</li>
    <li>Pesquisar sobre BPMN e sanar dúvidas com a professora.</li>
</ul>